...
const char frontslider_name[2][5][35] = {
  {"DEF_ROBOTR0.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTR1.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTR2.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTR3.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTR4.FRONTJOINT.FRONTSLIDER"},
  {"DEF_ROBOTB0.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTB1.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTB2.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTB3.FRONTJOINT.FRONTSLIDER", "DEF_ROBOTB4.FRONTJOINT.FRONTSLIDER"}
};

// plugin variables
static dGeomID robot_geom[2][5] = {{NULL, NULL, NULL, NULL, NULL}, {NULL, NULL, NULL, NULL, NULL}};
static dGeomID frontslider_geom[2][5] = {{NULL, NULL, NULL, NULL, NULL}, {NULL, NULL, NULL, NULL, NULL}};
...
void webots_physics_init() {
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < ROBOT_COUNT; j++) {
      ...
      frontslider_geom[i][j] = dWebotsGetGeomFromDEF(frontslider_name[i][j]);
      if (robot_geom[i][j] == NULL)
        dWebotsConsolePrintf("%s is missing.", robot_name[i][j]);
      if (frontslider_geom[i][j] == NULL)
        dWebotsConsolePrintf("%s is missing.", frontslider_name[i][j]);
    }
  }
