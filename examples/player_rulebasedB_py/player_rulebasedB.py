...
import helper
from players import Goalkeeper, Defender, Forward
...
class TestPlayer(Participant):
    def init(self, info):
        ...
        self.D1 = Defender(self.field, self.goal, self.penalty_area, 
                                self.goal_area, self.robot_size,
                                self.max_linear_velocity[self.d1_index])
        self.D2 = Defender(self.field, self.goal, self.penalty_area, 
                                self.goal_area, self.robot_size,
                                self.max_linear_velocity[self.d2_index])
        self.F1 = Forward(self.field, self.goal, self.penalty_area, 
                                self.goal_area, self.robot_size,
                                self.max_linear_velocity[self.f1_index])
        self.F2 = Forward(self.field, self.goal, self.penalty_area,
                                self.goal_area, self.robot_size,
        ...
        self.game_state = 0
                                ...
                                #   X ,    Y ,    TH
        self.default_formation =[-3.80,  0.50, 1.57,     # GK default: -3.80,  0.00, 1.57
                                 -1.75,  1.00, 0.00,     # D1 default: -2.25,  1.00, 0.00
                                 -1.75, -1.00, 0.00,     # D2 default: -2.25, -1.00, 0.00
                                 -1.75,  0.00, 0.00,     # F1 default: -0.65,  0.30, 0.00
                                 -0.65,  0.00, 0.00]     # F2 default: -0.65, -0.30, 0.00
